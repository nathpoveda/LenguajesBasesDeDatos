...
-- TABLA 1: CATEGORIAS
-- (para todo, acuerdense de revisar el archivo de nomenclatura del profesor. Está en el grupo de whatsapp por si acaso.)

-- Secuencia para categorias
-- Nota:Crear una secuencia para cada tabla
CREATE SEQUENCE  ID_CATEGORIAS_SEQ
START WITH 1
INCREMENT BY 1;

-- (este era el código de las tablas que ya estaba hecho)
...
-- Funcion para gestionar secuencia como tal.
...
-- Trigger para la secuencia del ID
...
-- Trigger para indicar la accion en la tabla
...
-- Trigger para auditar inserts y updates en la tabla 
