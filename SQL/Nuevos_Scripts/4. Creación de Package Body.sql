--
--       Declaración del Body del Package
--
CREATE
OR REPLACE PACKAGE BODY fide_proyecto_final_pkg AS --------------------------------------------------------------------------------------
...
BEGIN
INSERT INTO
    fide_estados_tb (nombre)
VALUES
    (v_nombre);

FOR estado IN insertar_estados LOOP IF LENGTH(estado.nombre) > 5 THEN DBMS_OUTPUT.PUT_LINE('Estado: ' || estado.nombre);
...
BEGIN
INSERT INTO
    fide_paises_tb (nombre)
VALUES
    (v_nombre);

FOR pais IN insertar_paises LOOP IF LENGTH(pais.nombre) > 7 THEN DBMS_OUTPUT.PUT_LINE('País: ' || pais.nombre);
...
) IS CURSOR insertar_proveedores IS
SELECT
    *
FROM
    fide_proveedores_tb
...
FOR proveedor IN insertar_proveedores LOOP DBMS_OUTPUT.PUT_LINE('Proveedor: ' || proveedor.nombre);
...
) IS CURSOR insertar_marcasproducto IS
SELECT
    *
FROM
    fide_marcasproducto_tb;

BEGIN
INSERT INTO
    fide_marcasproducto_tb (nombre)
VALUES
    (v_nombre);

FOR marca IN insertar_marcasproducto LOOP IF LENGTH(marca.nombre) > 15 THEN DBMS_OUTPUT.PUT_LINE('Marca: ' || marca.nombre);
...
FOR producto IN precio_productos LOOP IF producto.precio > 100 THEN DBMS_OUTPUT.PUT_LINE(
...
FOR proveedor IN proveedor_compramayor LOOP IF proveedor.preciocompra > 500 THEN DBMS_OUTPUT.PUT_LINE(
    'Proveedor: ' || proveedor.idproveedor || ' - Precio de compra: ' || proveedor.preciocompra
);
...
) IS CURSOR sucursales_cursor IS
SELECT
    *
FROM
    fide_sucursales_tb
...
FOR sucursal IN sucursales_cursor LOOP DBMS_OUTPUT.PUT_LINE(
    'Sucursal: ' || sucursal.nombre || ' - Hora de apertura: ' || sucursal.horaapertura
);
...
FOR inventario IN cantidad_inventario LOOP IF inventario.cantidad > 10 THEN DBMS_OUTPUT.PUT_LINE(
    'Inventario: Producto ' || inventario.idproducto || ' - Cantidad: ' || inventario.cantidad
);
...
FOR empleado IN empleados_cursor LOOP IF empleado.salario > 4000 THEN DBMS_OUTPUT.PUT_LINE(
    'Empleado: ' || empleado.nombre || ' - Salario: ' || empleado.salario || ' - Puesto: ' || empleado.puesto
);
...
FOR venta IN ventas LOOP IF(venta.total > 5000) THEN DBMS_OUTPUT.PUT_LINE(
    'Venta ID: ' || venta.idventa || ' - Total: ' || venta.total
);
...
) IS CURSOR detalleventas IS
SELECT
    *
FROM
    fide_detalleventas_tb
WHERE
    preciounitario > 500;
...
FOR detalle IN detalleventas LOOP IF(detalle.preciounitario > 500) THEN DBMS_OUTPUT.PUT_LINE(
    'Detalle de venta - Producto ID: ' || detalle.idproducto || ' - Precio Unitario: ' || detalle.preciounitario
);
...
BEGIN FOR estado IN estados_cursor LOOP
UPDATE
    fide_estados_tb
SET
...
DBMS_OUTPUT.PUT_LINE(
    'Estado con ID: ' || estado.idEstado || ' - Nuevo nombre: ' || v_nuevo_nombre
);
...
BEGIN FOR pais IN pais_cursor LOOP
UPDATE
    fide_paises_tb
SET
    nombre = v_nuevo_nombre
...
DBMS_OUTPUT.PUT_LINE(
    'País con ID: ' || pais.idpais || ' - Nuevo nombre: ' || v_nuevo_nombre
);
...
) IS CURSOR provincias_cursor IS
SELECT
    idprovincia,
    nombre
FROM
...
BEGIN FOR provincia IN provincias_cursor LOOP
UPDATE
    fide_provincias_tb
SET
    nombre = v_nuevo_nombre
...
DBMS_OUTPUT.PUT_LINE(
    'Provincia con ID: ' || provincia.idprovincia || ' - Nuevo nombre: ' || v_nuevo_nombre
);
...
BEGIN FOR canton IN cantones_cursor LOOP
UPDATE
    fide_cantones_tb
SET
    nombre = v_nuevo_nombre
...
DBMS_OUTPUT.PUT_LINE(
    'Cantón con ID: ' || canton.idcanton || ' - Nuevo nombre: ' || v_nuevo_nombre
);
...
BEGIN FOR distrito IN distritos_cursor LOOP
UPDATE
    fide_distritos_tb
SET
    nombre = v_nuevo_nombre
...
DBMS_OUTPUT.PUT_LINE(
    'Distrito con ID: ' || distrito.iddistrito || ' - Nuevo nombre: ' || v_nuevo_nombre
);
...
) IS CURSOR direcciones_cursor IS
SELECT
    iddireccion,
    direccionexacta
FROM
    fide_direcciones_tb
WHERE
    iddireccion = v_iddireccion;

BEGIN FOR direccion IN direcciones_cursor LOOP
UPDATE
    fide_direcciones_tb
SET
    direccionexacta = v_nueva_direccion
WHERE
    iddireccion = direccion.iddireccion;

DBMS_OUTPUT.PUT_LINE(
    'Dirección con ID: ' || direccion.iddireccion || ' - Nueva dirección: ' || v_nueva_direccion
);
...
) IS CURSOR proveedores_cursor IS
SELECT
    idproveedor,
    nombre
FROM
...
BEGIN FOR proveedor IN proveedores_cursor LOOP
UPDATE
    fide_proveedores_tb
SET
    nombre = v_nuevo_nombre
...
DBMS_OUTPUT.PUT_LINE(
    'Proveedor con ID: ' || proveedor.idproveedor || ' - Nuevo nombre: ' || v_nuevo_nombre
);
...
) IS CURSOR actualizar_marcasproducto IS
SELECT
    *
FROM
    fide_marcasproducto_tb
...
BEGIN FOR marca IN actualizar_marcasproducto LOOP
UPDATE
    fide_marcasproducto_tb
SET
    nombre = v_nuevo_nombre
...
IF LENGTH(marca.nombre) > 15 THEN DBMS_OUTPUT.PUT_LINE('Marca actualizada: ' || marca.nombre);
...
FOR producto IN precio_productos LOOP IF producto.precio > 100 THEN DBMS_OUTPUT.PUT_LINE(
    'Producto actualizado: ' || producto.nombre || ' - Nuevo precio: ' || producto.precio
);
...
BEGIN FOR proveedor IN proveedor_compramayor LOOP
UPDATE
    fide_productoproveedor_tb
SET
    preciocompra = v_nuevo_precio
WHERE
    idproducto = proveedor.idproducto;

DBMS_OUTPUT.PUT_LINE(
    'Producto ID: ' || proveedor.idproducto || ' actualizado con nuevo precio: ' || v_nuevo_precio
);
...
BEGIN FOR sucursal IN sucursales_cursor LOOP
UPDATE
    fide_sucursales_tb
SET
    nombre = v_nuevo_nombre,
...
DBMS_OUTPUT.PUT_LINE(
    'Sucursal con ID: ' || sucursal.idsucursal || ' actualizada a: ' || v_nuevo_nombre
);
...
PROCEDURE fide_inventarios_tb_actualizar_sp (
    ...
    v_ultimaactualizacion IN fide_inventarios_tb.ultimaactualizacion % TYPE,
    v_idsucursal IN fide_inventarios_tb.idsucursal % TYPE
) IS CURSOR inventarios_cursor IS
SELECT
    idproducto,
    cantidad,
    ultimaactualizacion,
...
BEGIN FOR inventario IN inventarios_cursor LOOP
UPDATE
    fide_inventarios_tb
SET
...
DBMS_OUTPUT.PUT_LINE(
    'Inventario actualizado: Producto ' || inventario.idproducto || ' - Nueva cantidad: ' || v_nueva_cantidad || ' - Última actualización: ' || v_ultimaactualizacion
);
...
BEGIN FOR empleado IN empleados_cursor LOOP
UPDATE
    fide_empleados_tb
SET
    salario = v_nuevo_salario,
    puesto = v_nuevo_puesto
WHERE
    idempleado = empleado.idempleado;

DBMS_OUTPUT.PUT_LINE(
    'Empleado actualizado: ' || empleado.nombre || ' - Nuevo salario: ' || v_nuevo_salario || ' - Nuevo puesto: ' || v_nuevo_puesto
);
...
BEGIN FOR venta IN ventas_cursor LOOP
UPDATE
    fide_ventas_tb
SET
    total = v_nuevo_total
...
DBMS_OUTPUT.PUT_LINE(
    'Venta con ID: ' || venta.idventa || ' - Nuevo total: ' || v_nuevo_total
);
...
) IS CURSOR detalleventas_cursor IS
SELECT
    idventa,
    idproducto,
    cantidad,
...
BEGIN FOR detalle IN detalleventas_cursor LOOP
UPDATE
    fide_detalleventas_tb
SET
    cantidad = v_nueva_cantidad,
...
DBMS_OUTPUT.PUT_LINE(
    'Detalle de venta actualizado - Producto ID: ' || detalle.idproducto || ' - Nueva cantidad: ' || v_nueva_cantidad || ' - Nuevo precio : ' || v_nuevo_precio
);
...
DBMS_OUTPUT.PUT_LINE(
    'Método de pago actualizado - ID: ' || v_idmetodopago || ' - Nuevo nombre: ' || v_nuevo_nombre
);
...
DBMS_OUTPUT.PUT_LINE(
    'Factura actualizada - ID Venta: ' || v_idventa || ' - Total: ' || v_total
);
...
SET
    ...
    motivo = v_motivo
WHERE
    iddevolucion = v_iddevolucion;

DBMS_OUTPUT.PUT_LINE(
    'Devolución actualizada - ID Devolución: ' || v_iddevolucion
);
...
-- "Eliminar" un pais
PROCEDURE FIDE_PAISES_TB_ELIMINAR_SP (V_idPais IN FIDE_PAISES_TB.idPais % TYPE) IS BEGIN
UPDATE
    FIDE_PAISES_TB
SET
    idEstado = 1
...
-- "Eliminar" una venta
PROCEDURE FIDE_VENTAS_TB_ELIMINAR_SP (V_idVenta IN FIDE_VENTAS_TB.idVenta % TYPE) IS BEGIN
UPDATE
    FIDE_VENTAS_TB
SET
    idEstado = 1
...
-- "Eliminar" un rol
PROCEDURE FIDE_ROLES_TB_ELIMINAR_SP (V_idRol IN FIDE_ROLES_TB.idRol % TYPE) IS BEGIN
UPDATE
    FIDE_ROLES_TB
SET
    idEstado = 1
...
--------------------------------------- FUNCIONES ------------------------------------
--------------------------------------------------------------------------------------
FUNCTION categorias_generar_secuencia_fn RETURN VARCHAR2 IS v_resultado VARCHAR2(100);

BEGIN v_resultado := id_categorias_seq.nextval;
...
BEGIN v_resultado := id_paises_seq.nextval;

RETURN v_resultado;

END paises_generar_secuencia_fn;

FUNCTION provincias_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_provincias_seq.nextval;

RETURN v_resultado;

END provincias_generar_secuencia_fn;

FUNCTION cantones_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_cantones_seq.nextval;

RETURN v_resultado;

END cantones_generar_secuencia_fn;

FUNCTION distritos_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_distritos_seq.nextval;

RETURN v_resultado;

END distritos_generar_secuencia_fn;

FUNCTION direcciones_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_direcciones_seq.nextval;

RETURN v_resultado;

END direcciones_generar_secuencia_fn;

FUNCTION proveedores_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_proveedores_seq.nextval;

RETURN v_resultado;

END proveedores_generar_secuencia_fn;

FUNCTION marcasproducto_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_marcasproducto_seq.nextval;

RETURN v_resultado;

END marcasproducto_generar_secuencia_fn;

FUNCTION productos_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_productos_seq.nextval;

RETURN v_resultado;

END productos_generar_secuencia_fn;

FUNCTION productoproveedor_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_productoproveedor_seq.nextval;

RETURN v_resultado;

END productoproveedor_generar_secuencia_fn;

FUNCTION sucursales_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_sucursales_seq.nextval;

RETURN v_resultado;

END sucursales_generar_secuencia_fn;

FUNCTION inventarios_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_inventarios_seq.nextval;

RETURN v_resultado;

END inventarios_generar_secuencia_fn;

FUNCTION empleados_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_empleados_seq.nextval;

RETURN v_resultado;

END empleados_generar_secuencia_fn;

FUNCTION ventas_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_ventas_seq.nextval;

RETURN v_resultado;

END ventas_generar_secuencia_fn;

FUNCTION detalleventas_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_detalleventas_seq.nextval;

RETURN v_resultado;

END detalleventas_generar_secuencia_fn;

FUNCTION metodospago_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_metodospago_seq.nextval;

RETURN v_resultado;

END metodospago_generar_secuencia_fn;

FUNCTION facturas_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_facturas_seq.nextval;

RETURN v_resultado;

END facturas_generar_secuencia_fn;

FUNCTION devoluciones_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_devoluciones_seq.nextval;

RETURN v_resultado;

END devoluciones_generar_secuencia_fn;

FUNCTION roles_generar_secuencia_fn RETURN NUMBER IS v_resultado NUMBER;

BEGIN v_resultado := id_roles_seq.nextval;
